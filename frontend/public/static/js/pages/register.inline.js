    ...
    async function handleRegister(e) {
      ...
      try {
        ...
        if (!res.ok) {
          // Backend NestJS trả {error:{status,message,detail,fields}}.
          // data.errors là format cũ của bản Django — giữ lại để tương thích.
          const fieldErrors = data.errors || (data.error && data.error.fields);
          if (fieldErrors && Object.keys(fieldErrors).length) {
            showFieldErrors(fieldErrors);
            // Vẫn hiện toast để người dùng không bỏ sót lỗi ở ô ngoài màn hình.
            showError(data.error && data.error.message ? data.error.message : 'Dữ liệu chưa hợp lệ.');
            return;
          }
