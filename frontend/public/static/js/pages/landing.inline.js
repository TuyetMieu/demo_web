document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('/api/courses')
      ...
      .then(res => {
        // /courses trả { ok, courses: [...] } — BỌC, không phải mảng trần.
        // Array.isArray(res) luôn false nên trang chủ KHÔNG BAO GIỜ hiện lưới
        // khóa học thật: mãi kẹt ở "Đăng nhập để xem khóa học" và số liệu cứng
        // "5+/225+", kể cả khi đã đăng nhập.
        const courses = Array.isArray(res) ? res : ((res && res.courses) || []);
        if (!courses.length) {
