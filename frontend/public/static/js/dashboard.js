...
/* ── Menu người dùng + chuông thông báo: đã chuyển sang edu-chrome.js ──
   Markup do HeaderBar.tsx dựng và dùng chung cho dashboard, trang chi tiết
   khóa học và 3 trang khóa CSDL, nên phần cài đặt cũng phải dùng chung —
   trước đây mỗi trang một bản, hai bản kia dùng thông báo viết cứng và ghi
   đè lên bản thật này khi điều hướng client-side. Xem đầu edu-chrome.js. */
...
(function () {
  ...
  /* Backend GET /skills trả MẢNG PHẲNG:
   *   { ok, skills: [{ courseId, skill, total, completed, progress }] }
   * (courses.service.ts getSkills). Còn giao diện dưới đây dựng theo cấu trúc
   * LỒNG của bản Flask cũ: skill_sets[].skills[].sub_skills[]. Đọc thẳng
   * `data.skill_sets` nên luôn là mảng rỗng → trang Kỹ năng lúc nào cũng hiện
   * "Chưa có dữ liệu kỹ năng", 4 ô tổng kết đều 0, và ô "Ôn tập hôm nay" ở
   * Bảng điều khiển luôn báo "Bạn đã ôn hết".
   *
   * Gom mảng phẳng theo khóa học cho khớp cấu trúc giao diện — không đổi gì ở
   * phần hiển thị. Vẫn nhận dạng cũ nếu backend đổi lại sau này.
   * Đặt trên window để edu-dashboard.js dùng chung, khỏi lệch luật đếm. */
  window.peAdaptSkills = function (data) {
    if (data && Array.isArray(data.skill_sets)) return data.skill_sets;
    var flat = (data && data.skills) || [];
    var byCourse = {}, order = [];
    flat.forEach(function (s) {
      var cid = s.courseId || 'khac';
      if (!byCourse[cid]) { byCourse[cid] = []; order.push(cid); }
      byCourse[cid].push(s);
    });
    return order.map(function (cid) {
      var mods = byCourse[cid];
      var meta = (window.courses || []).filter(function (c) { return c.id === cid; })[0] || {};
      var totalLessons = mods.reduce(function (n, m) { return n + (m.total || 0); }, 0);
      var doneLessons = mods.reduce(function (n, m) { return n + (m.completed || 0); }, 0);
      return {
        icon: meta.icon || '📘',
        title: meta.title || cid,
        progress: totalLessons ? Math.round((doneLessons * 100) / totalLessons) : 0,
        skills: mods.map(function (m) {
          var done = m.completed || 0, tot = m.total || 0;
          return {
            title: m.skill,
            progress: m.progress || 0,
            // backend không trả tên từng bài trong module → hiện số bài thay vì
            // danh sách bài, để phần mở rộng không rỗng trơ
            sub_skills: [{ title: done + '/' + tot + ' bài đã hoàn thành', done: tot > 0 && done >= tot }]
          };
        })
      };
    });
  ...
  function renderSkills(data) {
    var sets = window.peAdaptSkills(data);
...
(function () {
  ...
  /* Backend trả tên tác giả LỒNG trong `author: {id, name, avatar}` và mốc thời
   * gian là `createdAt` (camelCase). Bản trước đọc `row.author_name` /
   * `row.created_at` / `row.user_id` — cả ba đều không tồn tại, nên MỌI bài viết
   * và bình luận hiện tác giả "Ẩn danh", thời gian luôn là "vừa xong", và nút
   * "+ Theo dõi" không bao giờ hiện (userId rỗng). Đọc cả hai dạng tên. */
  function _apiAuthorName(row) {
    if (row.author && row.author.name) return row.author.name;
    return row.author_name || 'Ẩn danh';
  }
  function _apiAuthorId(row) {
    if (row.author && row.author.id != null) return row.author.id;
    return row.user_id != null ? row.user_id : null;
  }
  function _apiCreated(row) {
    return row.createdAt != null ? row.createdAt : row.created_at;
  }
  function _apiParentId(row) {
    return row.parentCommentId != null ? row.parentCommentId : (row.parent_comment_id || null);
  }

  function _apiPostToUi(row) {
    return {
      id: row.id, cat: row.category, title: row.title || '', body: row.content || '',
      userId: _apiAuthorId(row),
      author: _apiAuthorName(row), avatar: (row.author && row.author.avatar) || '🙋',
      time: _apiTime(_apiCreated(row)),
  ...
  function _apiCommentToUi(row) {
    return {
      id: row.id, author: _apiAuthorName(row), avatar: (row.author && row.author.avatar) || '🧑',
      time: _apiTime(_apiCreated(row)), text: row.content || '',
      reactions: row.reactions || _emptyReactions(), myReaction: row.my_reaction || null,
      parent_comment_id: _apiParentId(row), replies: []
    };
  }
  /* Dựng cây 1 cấp.
   * Backend ĐÃ lồng sẵn trả lời trong `comments[].replies` (forum.service.ts),
   * nên nếu chỉ duyệt phẳng theo parent id thì toàn bộ trả lời bị bỏ rơi —
   * không hiện lên bao giờ. Nhận cả hai: có `replies` thì dùng luôn, không thì
   * tự dựng cây từ mảng phẳng như trước. */
  function _buildCommentTree(flat) {
    var hasNested = flat.some(function (r) { return Array.isArray(r.replies) && r.replies.length; });
    if (hasNested) {
      return flat.map(function (row) {
        var node = _apiCommentToUi(row);
        node.replies = (row.replies || []).map(function (rep) {
          var child = _apiCommentToUi(rep);
    ...
    flat.forEach(function (row) {
      ...
      var pid = _apiParentId(row);
      if (pid && byId[pid]) {
        node.replyTo = byId[pid].author;
  ...
  window.toggleFollow = function (userId, btn) {
    ...
    fetch('/api/users/' + userId + '/follow', { method: isFollowing ? 'DELETE' : 'POST' })
      ...
      .then(function (d) {
        if (d && d.ok) {
          // Backend chỉ trả { ok: true }, KHÔNG có `following`. Đọc d.following
          // nên trạng thái luôn tính là "chưa theo dõi": bấm xong nút không đổi
          // chữ, và bấm lần nữa lại gửi POST thay vì DELETE. Suy ra từ hành
          // động vừa thực hiện; vẫn ưu tiên d.following nếu backend bổ sung sau.
          var nowFollowing = d.following != null ? !!d.following : !isFollowing;
          if (nowFollowing) _followingIds[userId] = true;
          ...
          document.querySelectorAll('.fpc-follow-btn[data-user-id="' + userId + '"]').forEach(function (b) {
            ...
            b.textContent = nowFollowing ? 'Đang theo dõi' : '+ Theo dõi';
          });
        }
      })
      .catch(function () {})
  ...
  /* renderPosts nằm TRONG IIFE này. forumSearch()/forumClearSearch() lại là hàm
   * top-level (cuối file) và gọi thẳng renderPosts() — ngoài tầm nhìn, nên mỗi
   * lần gõ vào ô "Tìm bài viết..." là ném ReferenceError: renderPosts is not
   * defined, danh sách không lọc lại lần nào. Đưa ra window như các hàm forum*
   * khác trong IIFE để hai hàm kia gọi được. */
  window.forumRenderPosts = renderPosts;
...
(function () {
  ...
  function _renderXPChart() {
    ...
    fetch('/api/stats/xp-by-course')
      .then(function (r) { return r.json(); })
      .then(function (d) {
        // Backend trả { ok, xp_by_course: [{courseId, xp}] } — không phải
        // `subjects`, và mỗi phần tử KHÔNG có `title`. Đọc sai nên biểu đồ XP ở
        // trang Cá nhân lúc nào cũng báo "Chưa có XP nào". Lấy tên khóa từ
        // window.courses (main.js đã nạp), thiếu thì dùng mã khóa.
        var raw = (d && (d.xp_by_course || d.subjects)) || [];
        var subjects = raw.map(function (s) {
          var meta = (window.courses || []).filter(function (c) { return c.id === s.courseId; })[0] || {};
          return { courseId: s.courseId, xp: s.xp || 0, title: s.title || meta.title || s.courseId };
  ...
  function _loadProfile() {
    ...
    fetch('/api/stats')
      .then(function (r) { return r.json(); })
      // /stats trả `streak`, không phải `streakDays` — đọc sai nên ô "Chuỗi
      // ngày học" ở trang Cá nhân trống trơn (undefined) suốt.
      .then(function (d) { _set('prof-streak', d.streak != null ? d.streak : (d.streakDays || 0)); })
    ...
    // /achievements trả { ok, achievements: [{…, unlocked}] } — KHÔNG có
    // unlockedCount/totalCount, nên điều kiện cũ không bao giờ đúng và ô
    // "Thành tích đạt được" đứng nguyên ở "—". Tự đếm từ mảng.
    fetch('/api/achievements')
      .then(function (r) { return r.json(); })
      .then(function (d) {
        var list = (d && d.achievements) || [];
        if (!list.length) return;
        var unlocked = list.filter(function (a) { return a.unlocked || a.earned || a.achieved; }).length;
        _set('prof-achievements', unlocked + '/' + list.length);
      })
      .catch(function () { /* giữ nguyên '—' nếu lỗi */ });

    // Kỹ năng: đếm thật từ API /api/skills (trước đây scrape #sk-grid,
    // chỉ có sau khi user mở trang Kỹ năng nên thường trống).
    // Dùng chung bộ chuyển đổi với trang Kỹ năng — /skills trả mảng phẳng.
    fetch('/api/skills')
      .then(function (r) { return r.json(); })
      .then(function (d) {
        var total = 0;
        window.peAdaptSkills(d).forEach(function (bs) {
...
function forumSearch(val) {
  ...
  // renderPosts nằm trong IIFE diễn đàn — gọi qua window (xem forumRenderPosts)
  if (typeof window.forumRenderPosts === 'function') window.forumRenderPosts();
}
