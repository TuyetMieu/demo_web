...
var COURSE_URLS = {
  ...
  // Hai khoá này ĐÃ có trang học (route động /lesson/[courseId] xử lý cả ba khoá
  // db_design*), nhưng trước đây thiếu ở bảng này nên nút "Tiếp tục học" không
  // gắn được sự kiện bấm — đăng ký xong vẫn không vào học được.
  db_design_tc: "/lesson/db_design_tc",
  db_design_nc: "/lesson/db_design_nc",
};

/* Khoá học không có trong bảng trên vẫn phải vào được trang chi tiết, thay vì
 * nút bấm không phản ứng gì. */
function courseStudyUrl(courseId) {
  return COURSE_URLS[courseId] || (courseId ? "/courses/" + courseId : "#");
}

/* Điều hướng trong app — bản đầy đủ kèm chú thích nằm ở edu-chrome.js (file đó
 * dùng chung cho cả trang không nạp main.js). Ở đây chỉ khai báo nếu chưa có,
 * để các trang chỉ nạp main.js (bài học, /interface) vẫn có hàm này. */
if (typeof window.peGo !== "function") {
...
function renderCourses() {
  ...
  // Thẻ khóa học dựng theo màn "Khóa học" của Programming EDU.dc.html:
  // ảnh vân chéo tối 150px → danh mục → tên → mô tả → meta → thanh tiến độ.
  // Chỉ đổi phần HTML; toàn bộ lọc/sắp xếp/tìm kiếm ở trên giữ nguyên.
  //
  // KHÓA ĐÃ ĐĂNG KÝ (2026-08-12): design bản tối bọc thanh tiến độ trong
  // `sc-if c.enrolled` — khóa chưa đăng ký KHÔNG có thanh. Trước đây ta vẽ
  // thanh cho mọi thẻ nên khóa chưa đăng ký hiện một vạch xám rỗng, trông như
  // phần tử hỏng/đang tải. Giờ chỉ khóa đã đăng ký mới có, kèm dòng nhãn
  // "Tiến độ / N%" cho biết con số (design chỉ có thanh trần, nhưng ở đây một
  // thẻ vừa là lối vào trang chi tiết nên con số là thông tin thật sự hữu ích).
  // Nhãn góc ảnh cũng đổi từ xanh lá (lạc tông) sang gradient tím của design.
  //
  // BẤM THẺ (2026-08-12): kể cả khóa đã đăng ký cũng vào trang CHI TIẾT
  // (/courses/<id>), không nhảy thẳng vào bài học nữa — trước đây bấm thẻ
  // đã đăng ký là vào bài học ngay, không kịp xem tiến độ/nội dung khóa. Vào
  // bài học giờ chỉ còn qua nút "Tiếp tục học" trong trang chi tiết đó
  // (courseStudyUrl vẫn dùng cho nút đó, xem goLesson() ở course_detail.js).
  grid.innerHTML = filtered
    .map(function (c, i) {
      var pct = Math.max(0, Math.min(100, Math.round(c.progress || 0)));
      var goUrl = "/courses/" + c.id;
      return [
        ...
        "</div>",
        '<div class="edu-c-body">',
        '<div class="edu-c-cat">' + (c.tag || "") + "</div>",
        '<div class="edu-c-title">' + c.title + "</div>",
        '<p class="edu-c-desc">' + c.description + "</p>",
        '<div class="edu-c-meta">',
        "<span>" + c.lessons + " bài</span><span>·</span><span>" + c.level + "</span><span>·</span><span>" + c.duration + "</span>",
        "</div>",
        c.enrolled
          ? '<div class="edu-c-prog">' +
            '<div class="edu-c-prog-head"><span>Tiến độ</span><span class="edu-c-prog-pct">' + pct + '%</span></div>' +
            '<div class="edu-c-track"><div class="edu-c-fill" style="width:' + pct + '%"></div></div>' +
            "</div>"
          : "",
        "</div>",
        "</div>",
    ...
    .map(function (c) {
      return [
        ...
        '<button class="continue-btn"' +
          ...
          c.color +
          '40"' +
          " onclick=\"window.location='" + courseStudyUrl(c.id) + "'\"" +
          ">▶ Tiếp tục học</button>",
...
function _applyEnrollState(courseId, enrolled) {
  ...
  if (enrolled) {
    ...
    if (c && !enrolledCourses.some(function(e) { return e.id === courseId; })) {
      // Đẩy vào một bản ghi ĐÚNG DẠNG thẻ "khoá học của tôi".
      // Trước đây đẩy nguyên object khoá học vào, làm danh sách trộn hai kiểu dữ
      // liệu khác nhau: thiếu progress/completedLessons/timeSpent nên thẻ hiện
      // "undefined". Các trường tiến độ để 0 vì vừa đăng ký xong; lần tải lại
      // trang sau sẽ lấy số thật từ API.
      enrolledCourses.push({
        ...
        subtitle: c.subtitle || '',
        duration: c.duration || '',
        color: c.color || c.accentColor || '#3B82F6',
        accentColor: c.accentColor || c.color || '#3B82F6',
        icon: c.icon || '📘',
        progress: 0,
        completedLessons: 0,
...
function loadStats() {
  return fetch(API + "/stats")
    ...
    .then(function (s) {
      ...
      // GET /stats trả `streak` và `coursesEnrolled` (xem stats.service.ts).
      // Trước đây đọc `streakDays`/`enrolledCount` — hai trường KHÔNG tồn tại,
      // nên chuỗi ngày học hiện "undefined ngày", số khóa đang học luôn trống
      // và lịch học tuần không tô ngày nào. Nhận cả tên cũ phòng backend đổi lại.
      var streakVal = s.streak != null ? s.streak : s.streakDays;
      var enrolledVal = s.coursesEnrolled != null ? s.coursesEnrolled : s.enrolledCount;
      setText("stat-enrolled", enrolledVal);
      setText("stat-total-hours", s.totalHours);
      setText("stat-streak", (streakVal || 0) + " ngày");
      setText("stat-certificates", s.certificates);
      setText("my-stat-count", enrolledVal);
      setText("my-stat-hours", s.totalHours);
      setText("my-stat-avg", s.avgProgress + "%");
      renderStudyCalendar(streakVal || 0, s.streakActive || false);
    });
}
...
function loadNotifications() {
  return fetch(API + "/notifications")
    ...
    .then(function (n) {
      if (!n) return;
      // Backend trả { ok, settings: {...} } — bọc trong `settings`, không phẳng
      // (notifications.service.ts getSettings). Đọc thẳng n.emailNotif nên cả 4
      // công tắc trong Cài đặt luôn hiện TẮT dù đã bật, và bấm Lưu sẽ ghi đè
      // đúng giá trị sai đó xuống DB.
      var s = n.settings || n;
      setToggle("toggle-email", s.emailNotif);
      setToggle("toggle-push", s.pushNotif);
      setToggle("toggle-remind", s.studyRemind);
      setToggle("toggle-content", s.contentUpdate);
    });
}
...
function _applyCoursesData(data) {
  ...
  // Danh sách khóa học KHÔNG kèm tiến độ: mapCourse() ở backend chỉ trả
  // id/title/level/lessons/… chứ không có `progress` (courses.service.ts).
  // Thẻ khóa học lại đọc c.progress nên khóa đang học luôn hiện "Tiến độ 0%"
  // với thanh rỗng — dù mục "Bài học gần đây" ngay bên cạnh hiện đúng 44%.
  // Tiến độ thật nằm ở /enrolled; ghép sang theo id.
  var progressById = {};
  (enrolledCourses || []).forEach(function (e) {
    var key = e.id != null ? e.id : e.courseId;
    if (key != null) progressById[key] = e;
  });
  courses.forEach(function (c) {
    var e = progressById[c.id];
    if (!e) return;
    if (c.progress == null) c.progress = e.progress || 0;
    if (c.completedLessons == null) c.completedLessons = e.completedLessons || 0;
    if (!c.enrolled) c.enrolled = true;
  });
...
function applyTheme(isDark) {
  ...
  // Nút cũ chỉ chứa emoji nên ghi đè textContent được. Sidebar mới dựng nút
  // bằng <svg> + <span> nhãn — ghi đè sẽ xoá sạch, nên bỏ qua nút có phần tử con.
...
function _peInitMain() {
  ...
  // Mở đúng tab khi vừa tới dashboard. Ưu tiên __pePendingTab (peGoTab() ở
  // edu-chrome.js ghi biến này rồi điều hướng client-side, KHÔNG qua hash —
  // xem chú thích ở peGoTab để biết vì sao); location.hash chỉ còn dùng cho
  // điều hướng thật sự nạp lại trang (gõ thẳng /dashboard#roadmap, bookmark…).
  var pendingTab = window.__pePendingTab;
  if (pendingTab) delete window.__pePendingTab;
  var hash = pendingTab || window.location.hash.replace('#', '');
  var validPages = ['dashboard', 'courses', 'roadmap', 'skills', 'forum', 'leader', 'settings', 'profile', 'my-courses'];
...
document.addEventListener("DOMContentLoaded", _peInitMain);

/* Quay lại dashboard bằng client-side routing: React dựng lại toàn bộ markup
 * nhưng file này KHÔNG được nạp lại (LegacyScripts lọc trùng theo src) nên
 * DOMContentLoaded không bao giờ bắn lần hai — dashboard sẽ trống trơn: lưới
 * khóa học rỗng, tên người dùng "—", mọi ô thống kê trắng. Trang React bắn
 * sự kiện này khi mount LẠI để chạy đúng phần khởi tạo đó.
 * loadAll() chỉ fetch rồi render nên gọi lại nhiều lần vẫn an toàn. */
document.addEventListener("pe:page-remount", _peInitMain);
