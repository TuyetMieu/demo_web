...
(function applyCourseContent() {
  ...
  // Tiêu đề + phụ đề trên HeaderBar do REACT dựng, nên đưa dữ liệu cho React
  // thay vì tự ghi vào DOM: HeaderBar còn render lại khi dữ liệu người dùng về
  // (và khi tắt vệt chờ), lúc đó React đặt lại nội dung theo prop của nó và xoá
  // sạch giá trị JS vừa ghi — đo được: mở khóa Nâng cao mà tiêu đề trang quay
  // về "Database Design" của khóa Cơ bản.
  // Phụ đề là DÒNG META chứ không phải heroSubtitle: hero ngay bên dưới đã hiện
  // heroSubtitle rồi, đặt lại y hệt thì câu mô tả dài hiện lặp hai lần.
  window.__peDbCourseTitle = CD_META.heroTitle;
  window.__peDbCourseLessons = CD_META.lessonCount;
...
(function renderRoadmap() {
  ...
  /* Báo cho CourseDbDesign.tsx biết nội dung thật đã đổ xong.
   *
   * Markup trong JSX là bản MẶC ĐỊNH của khóa Cơ bản (tên "Database Design",
   * 13 thành tựu, 4 yêu cầu, #cd-roadmap rỗng). File này mới thay bằng nội dung
   * đúng khóa. Không có tín hiệu thì React hiện bản mặc định trước rồi mới bị
   * thay — mở khóa Nâng cao vẫn thấy tên khóa Cơ bản nhấp nháy một nhịp (đo
   * được ở mốc 160ms). React che bằng vệt chờ cho tới khi nhận tín hiệu này.
   *
   * Đặt cả CỜ lẫn SỰ KIỆN: React có thể mount sau khi script đã chạy xong
   * (bỏ lỡ sự kiện) — lúc đó nó đọc cờ. */
  window.__peDbCourseReady = true;
  document.dispatchEvent(new Event('pe:db-course-ready'));
})();
...
function _renderCtaEnrolled(completedLessons) {
  ...
  // Bộ class .edu-cd-* — PHẢI khớp với markup React trong CourseDbDesign.tsx,
  // nếu không thì sau khi bấm "Đăng ký" thẻ này đổi hẳn kiểu so với lúc mới vào.
  area.innerHTML =
    '<div class="edu-cd-progress-row">' +
      '<div class="edu-cd-progress-top">' +
        '<span class="edu-cd-progress-pct">' + pct + '%</span>' +
        '<span class="edu-cd-progress-frac">' + done + '/' + total + ' bài</span>' +
      '</div>' +
      '<div class="edu-cd-progress-bar"><div class="edu-cd-progress-fill" style="width:' + pct + '%"></div></div>' +
    '</div>' +
    '<div class="edu-cd-stats">' +
      '<div class="edu-cd-stat"><div class="v">' + done + '</div><div class="l">Bài đã học</div></div>' +
      '<div class="edu-cd-stat"><div class="v">' + (total - done) + '</div><div class="l">Còn lại</div></div>' +
      '<div class="edu-cd-stat"><div class="v">0h</div><div class="l">Đã học</div></div>' +
      '<div class="edu-cd-stat"><div class="v">' + streak + '</div><div class="l">🔥 Streak</div></div>' +
    '</div>' +
    '<button type="button" class="edu-cd-btn primary" style="width:100%;margin-top:18px" onclick="goLesson()">▶ Bắt đầu học</button>' +
    '<button type="button" class="edu-cd-unenroll" id="unenroll-btn" onclick="unenroll()">Hủy đăng ký</button>';
}
...
function _renderCtaGuest() {
  ...
  if (!area) return;
  area.innerHTML =
    '<button type="button" class="edu-cd-btn primary" style="width:100%;margin-top:18px" id="enroll-btn" onclick="enroll()">' +
...
/* ── Menu người dùng + chuông thông báo: đã chuyển sang edu-chrome.js ──
   Y hệt course_detail.js: bản chép ở đây dùng thông báo VIẾT CỨNG và ghi đè
   bản thật khi điều hướng client-side. Xem đầu edu-chrome.js. */
